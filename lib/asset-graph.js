...
var qkfiles = require('@quakejs/formats');
...
AssetGraph.prototype.addMap = function (name, game, buffer, whitelist) {
	...
	// @quakejs/formats uses string lump names instead of numeric constants
	var map = qkfiles.bsp.load(buffer, { lumps: ['entities', 'shaders'] });
...
AssetGraph.prototype.addScript = function (name, game, buffer) {
	...
	// qkfiles.shader.load parses a whole .shader file into a { name: text } dictionary
	var script = qkfiles.shader.load(buffer.toString('utf8'));
...
AssetGraph.prototype.addShader = function (name, game, buffer) {
	...
	// qkfiles.script.load parses a single shader block into a Script object with .stages, .innerBox, .outerBox
	var shader = qkfiles.script.load(buffer);
